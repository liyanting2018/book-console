...
(function () {
    angular.module("w5c.validator")
        ...
        .directive('w5cDynamicName', [function () {
            return {
                ...
                link    : function (scope, elm, attrs, ngModelCtr) {
                    var name = scope.$eval(attrs.w5cDynamicName) || "name_"+Math.random().toString(10).substr(2,10);
                    ngModelCtr.$name = name;
                    elm.attr('name', name);
        ...
        .directive('w5cValidatorElement', ["$timeout", function ($timeout) {
            return {
                restrict: 'A',
                require : ["?ngModel", "?^w5cFormValidate", "?^form"],
                link    : function (scope, elm, attrs, ctrls) {
                    var ngModelCtr = ctrls[0];
                    var name = attrs.name || "name_"+Math.random().toString(10).substr(2,10);
                    ngModelCtr.$name = name;
                    elm.attr('name', name);
                    var formController = elm.controller('form') || {
                            $addControl: angular.noop
